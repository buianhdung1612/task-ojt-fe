...
export const FilterDetail = () => {
    ...
    const fetchApi = () => {
        fetch(`https://task-ojt.onrender.com/tasks?status=${status}`, {
            headers: { Authorization: `Bearer ${token}` }
        })
            .then(res => res.json())
            .then(data => {
    ...
    return (
        <>
            <Link to="/filters" className="text-[#666] rounded-[5px] mb-[10px] text-center cursor-pointer w-[80px] hover:bg-[#f5f5f5] hover:text-[#1a1a1a]">Filters /</Link>
            {status == "initial" && (
                <Title title="To do" />
            )}
            {status == "doing" && (
